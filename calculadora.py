...
class Calculator:
    ...
    def area_superficie_esfera(self, radio):
        """Calcula el área de la superficie de una esfera dado su radio"""
        if radio < 0:
            raise ValueError("El radio no puede ser negativo")
        resultado = 4 * math.pi * radio ** 2
        self.historial.append(f"surface_area_sphere(r={radio}) = {resultado}")
    ...
    def perimetro_sector_circular(self, radio, angulo):
        """Calcula el perímetro de un sector circular dado radio y ángulo en grados"""
        if radio < 0 or angulo < 0 or angulo > 360:
            raise ValueError("Radio debe ser positivo y ángulo entre 0 y 360 grados")
        arco = (angulo / 360) * 2 * math.pi * radio
        resultado = 2 * radio + arco
        self.historial.append(f"perimeter_circular_sector(r={radio}, angle={angulo}°) = {resultado}")
        return resultado
    
    def area_segmento_circular(self, radio, angulo):
        """Calcula el área de un segmento circular dado radio y ángulo en grados"""
        if radio < 0 or angulo < 0 or angulo > 360:
            raise ValueError("Radio debe ser positivo y ángulo entre 0 y 360 grados")
        area_sector = (angulo / 360) * math.pi * radio ** 2
        area_triangulo = (radio ** 2 * math.sin(math.radians(angulo))) / 2
        resultado = area_sector - area_triangulo
    ...
    def area_poligono_regular(self, n, lado):
        """Calcula el área de un polígono regular dado número de lados y longitud del lado"""
        if n < 3 or lado <= 0:
            raise ValueError("Número de lados debe ser >= 3 y lado > 0")
        resultado = (n * lado ** 2) / (4 * math.tan(math.pi / n))
    ...
    def perimetro_anillo(self, radio_exterior, radio_interior):
        """Calcula el perímetro de un anillo dado radio exterior y radio interior"""
        if radio_exterior <= 0 or radio_interior < 0 or radio_interior >= radio_exterior:
            raise ValueError("Radio exterior debe ser positivo y mayor que radio interior")
        resultado = 2 * math.pi * (radio_exterior + radio_interior)
    ...
    def volumen_icosaedro(self, arista):
        """Calcula el volumen de un icosaedro regular dado su arista"""
        if arista <= 0:
            raise ValueError("La arista debe ser positiva")
        resultado = (5/12) * (3 + math.sqrt(5)) * arista ** 3
        self.historial.append(f"volume_regular_icosahedron(a={arista}) = {resultado}")
        return resultado
    
    def area_superficie_icosaedro(self, arista):
        """Calcula el área de la superficie de un icosaedro regular dado su arista"""
        if arista <= 0:
            raise ValueError("La arista debe ser positiva")
        resultado = 5 * math.sqrt(3) * arista ** 2
        self.historial.append(f"surface_area_regular_icosahedron(a={arista}) = {resultado}")
        return resultado
    
    def area_sector_esferico(self, radio, angulo):
        """Calcula el área de un sector esférico dado radio y ángulo en grados"""
        if radio <= 0 or angulo < 0 or angulo > 360:
            raise ValueError("Radio positivo y ángulo entre 0 y 360")
        resultado = (angulo / 360) * 4 * math.pi * radio ** 2
    ...
    def area_zona_esferica(self, radio, altura):
        """Calcula el área de una zona esférica dado radio y altura"""
        if radio <= 0 or altura <= 0 or altura > 2 * radio:
            raise ValueError("Radio positivo y altura entre 0 y 2*radio")
        resultado = 2 * math.pi * radio * altura
    ...
    def volumen_cono(self, radio, altura):
        """Calcula el volumen de un cono: (1/3) * π * r² * h"""
        if radio <= 0 or altura <= 0:
            raise ValueError("Radio y altura deben ser positivos")
        resultado = (1/3) * math.pi * radio ** 2 * altura
